...
unsafe extern "C" fn pvPortMalloc(xWantedSize: usize) -> *mut c_void {
    unsafe {
        ...
        let wrap_ptr = alloc::alloc::alloc(wrap_layout);
        if wrap_ptr.is_null() {
            return core::ptr::null_mut();
        }
...
unsafe extern "C" fn pvPortCalloc(xNum: usize, xSize: usize) -> *mut c_void {
    unsafe {
        let Some(size) = usize::checked_mul(xNum, xSize) else {
            return core::ptr::null_mut();
        };
        if size == 0 {
            return core::ptr::null_mut();
        ...
        let wrap_ptr = alloc::alloc::alloc_zeroed(wrap_layout);
        if wrap_ptr.is_null() {
            return core::ptr::null_mut();
        }
        let ptr = wrap_ptr.add(offset);
...
unsafe extern "C" fn vPortFree(x: *mut c_void) {
    unsafe {
        ...
        alloc::alloc::dealloc(wrap_ptr, wrap_layout)
    }
}
